...
export default class Controller {
    ...
    public hasCommand<T extends SimpleCommand>(notificationName: string, commandClassRef: new () => T): boolean {
        return this.hasAnyCommand(notificationName) && this.commandMap[notificationName].indexOf(commandClassRef) !== -1;
    }

    public hasAnyCommand(notificationName: string): boolean {
        return this.commandMap[notificationName] !== undefined && this.commandMap[notificationName].length !== 0;
    }
    ...
    protected executeCommand<A extends FunctionArgs>(notificationName: string, ...args: A): void {
        const commandClassRefs: (new () => SimpleCommand)[] = this.commandMap[notificationName];
        ...
        for (const commandClassRef of commandClassRefs) {
            ...
            commandInstance.initializeNotifier(this.singletonInstanceKey);
